...
create table Historico_pedidos(
    ...
    CFK_prod int,
    CFK_User int,
    constraint FK_users foreign key (CFK_User) references usuario(Cod_usuario),
    constraint FK_prods foreign key (CFK_prod)references produto(Cod_prod)
);

create table carrinho(
	Cod_carr int primary key auto_increment,
    CFK_prod int,
    constraint FK_prid foreign key (CFK_prod) references produto(Cod_prod)
);
